...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        channel_num=self.n_channels
        k=create_Gaussian_kernel_2D(cutoff_frequency=cutoff_frequency)
        rows,cols=k.shape
        k=k.reshape((1,rows,cols))
        k=np.tile(k,(channel_num,1,1,1))
        kernel=torch.Tensor(k)
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        # )
        _,_,k_size,_=kernel.shape
        print("padding",int((k_size-1)/2))
        filtered_image=F.conv2d(x,kernel,padding=int((k_size-1)/2),groups=self.n_channels)
        ### END OF STUDENT CODE ####
        ############################
    ...
    def forward(
        ...
        print("n_channles",self.n_channels)
